// AUDIT FIX: LoginPage.tsx - Gunakan backend JWT, hapus hardcoded credentials
...
export const LoginPage: React.FC<LoginPageProps> = ({ onBack, onLoginSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // AUDIT FIX: Kirim ke backend API, bukan compare hardcoded credentials
  ...
  return (
    <div className="min-h-screen bg-[#0d1655] flex items-center justify-center p-4 font-sans text-white relative overflow-hidden">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        <div className="absolute top-[-10%] right-[-10%] w-[50%] h-[50%] rounded-full bg-gradient-to-br from-[#F47920]/20 to-transparent blur-[120px]" />
        <div className="absolute bottom-[-10%] left-[-10%] w-[50%] h-[50%] rounded-full bg-gradient-to-tr from-[#1a2d8f]/30 to-transparent blur-[120px]" />
      </div>
        ...
        <div className="bg-white rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.3)] border border-white/20 p-6 sm:p-10">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button type="submit" disabled={loading} className="w-full relative group">
              <div className="absolute -inset-1 bg-gradient-to-r from-[#F47920] to-orange-600 rounded-[1.5rem] blur opacity-40 group-hover:opacity-100 transition duration-500" />
